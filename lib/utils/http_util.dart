...
class HttpUtil {
  ...
  static Future<HttpUtil> getInstance() async {
    ...
    if (null == instance) instance = new HttpUtil(token);
    return instance;
  }
  ...
  HttpUtil(String token)  {
    ...
    options = new BaseOptions(
      //请求基地址,可以包含子路径
      // baseUrl: "http://app.cyogaschool.com:8080",
...
class HttpUtilDownApk {
  static HttpUtilDownApk instance;
  Dio dio;
  BaseOptions options;

  CancelToken cancelToken = new CancelToken();

  static Future<HttpUtilDownApk> getInstance() async {
    String token = await SharedPreferenceUtil.query("token");
    if (null == instance) instance = new HttpUtilDownApk(token);
    return instance;
  }
   ...
   * config it and create
   */
  HttpUtilDownApk(String token)  {
    //BaseOptions、Options、RequestOptions 都可以配置参数，优先级别依次递增，且可以根据优先级别覆盖参数
    options = new BaseOptions(
      //请求基地址,可以包含子路径
      baseUrl: "http://app.cyogaschool.com",
      // baseUrl: "http://10.0.2.2:8080",
      //连接服务器超时时间，单位是毫秒.
      connectTimeout: 10000,
      //响应流上前后两次接受到数据的间隔，单位为毫秒。
